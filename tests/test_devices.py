...
def test_get_managed_devices_all(
    ...
    devices_all = json.loads(
        read_fixture(
            f"aos/{aos_api_version}/devices/" "get_managed_devices_all.json"
        )
    )
    aos_session.add_response(
        "GET", "http://aos:80/api/systems", status=200, resp=json.dumps(devices_all)
    )
...
def test_get_managed_devices_by_id(
    ...
    device = json.loads(
        read_fixture(f"aos/{aos_api_version}/devices/" "get_managed_devices_id.json")
    )
...
def test_get_system_agents_all(
    ...
    aos_session.add_response(
        ...
        status=200,
        resp=read_fixture(
            f"aos/{aos_api_version}/devices/" f"get_sys_agents_all.json"
        ),
    )
